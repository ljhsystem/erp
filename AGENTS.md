...
- 현재 재고관리 범위는 수량 수불부·창고·입출고 시스템이 아니라 회계연도별 재고가액 증감 근거 관리로 제한한다.
- 재고가액 계약은 `기말재고 = 기초재고 + 당기 증가 - 당기 감소`로 고정한다.
- 재고행은 공용 `BUSINESS_UNIT`을 사용하며 전문건설업(`CONSTRUCTION`)은 프로젝트를 필수로 한다. 본사와 통신판매업은 프로젝트를 선택사항으로 둔다.
- 모든 재고행은 품명, 재고구분, 산출근거와 증거자료 참조를 보유해야 한다. 근거 없는 합계금액 직접 입력은 허용하지 않는다.
- 재고를 `추정재고`, `현행재고`로 표시하지 않는다. 화면과 API는 기초재고, 당기 증가, 당기 감소, 당기 증감, 기말재고 명칭을 사용한다.
- 품목마스터·창고·수불수량·평가방법은 별도 업무 Closure가 승인되기 전까지 재고가액 관리에 추가하지 않는다.
...
- 현장관리의 페이지 소유 업무 테이블은 `site_*`을 사용하며, 하위 도메인은 `site_{domain}_*` 형식으로 확장한다. 예: 영업관리 `site_sales_*`.
...
버그 수정 시 동일 기능의 정상 동작 화면이 존재하면,
추측으로 수정하기 전에 정상 화면와 비정상 화면을 먼저 비교한다.
...
- 정상 화면가 있는데 추측으로 공용 로직 수정
- 동일한 가설을 반복 검증
- 실험용 토글 반복 추가
- 임시 CSS 우회
- 원인 미확정 상태에서 공용 코드 수정

------------------------------------------------

시간 제한

버그 원인 분석은 최대 30분으로 제한한다.
