...
export function createAddRowCommand(payload = {}) {
    return {
        type: 'add-row',
        payload,
        canExecute(context = {}) {
            return Boolean(context.state) && isCapabilityEnabled(context.state, 'addRow');
        },
        execute(context = {}) {
            ...
            requestDerivedState(context, 'add-row', { row, rowIndex });
            context.eventBus?.emit('row:added', { row, rowIndex });
