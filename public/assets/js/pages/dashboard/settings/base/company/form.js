export function createCompanyFormModule({
    wrapper,
    saveButton,
    notify,
    api,
    ...
    function collectFormData() {
        ...
        if (foundDateInput) {
            foundDateInput.value = normalizeDateInputValue(foundDateInput.value, notify);
        }
    ...
    function validateFormData(data) {
        if (!data.company_name_ko.trim()) return '회사명(국문)은 필수입니다.';
        const bizNumber = digitsOnly(data.biz_number);
        if (bizNumber && bizNumber.length !== 10) return '사업자등록번호는 숫자 10자리여야 합니다.';
        const corpNumber = digitsOnly(data.corp_number);
        if (corpNumber && corpNumber.length !== 13) return '법인등록번호는 숫자 13자리여야 합니다.';
        const tel = digitsOnly(data.tel);
        if (tel && (tel.length < 9 || tel.length > 11)) return '전화번호 형식이 올바르지 않습니다.';
        const fax = digitsOnly(data.fax);
        if (fax && (fax.length < 9 || fax.length > 11)) return '팩스번호 형식이 올바르지 않습니다.';
        if (data.tax_email && !isValidEmail(data.tax_email)) return '세금계산서 이메일 형식이 올바르지 않습니다.';
        if (data.sub_email && !isValidEmail(data.sub_email)) return '서브 이메일 형식이 올바르지 않습니다.';
        if (data.company_website && !isValidUrl(data.company_website)) return '홈페이지 주소 형식이 올바르지 않습니다.';
        if (data.sns_instagram && !isValidInstagram(data.sns_instagram)) return '인스타그램은 URL 또는 계정명 형식으로 입력해주세요.';
        return '';
    }
    ...
    function normalizeDate(value) {
        ...
        return normalizeDateInputValue(value, notify);
    }
