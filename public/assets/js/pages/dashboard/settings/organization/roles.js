...
(() => {
    ...
    function bindTableEvents($) {
        ...
        $('#role-table tbody')
            .off('click.roleCellSearch', 'td')
            .on('click.roleCellSearch', 'td', function () {
                const cell = roleTable.cell(this);
                const idx = cell.index();
                if (!idx) return;

                const field = roleTable.column(idx.column).dataSrc();
                if (!field || field === 'is_active') return;

                const value = cell.data();
                const $first = $('#roleSearchConditions .search-condition').first();
