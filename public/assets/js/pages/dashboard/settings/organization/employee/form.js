...
const EMPLOYEE_MODAL_FIELD_POLICIES = Object.freeze([
    ...
    { selector: '#edit_representative_qualification_file', key: 'representative_qualification_id', fallback: '대표 자격증' },
...
function isEmployeeFilePolicyKey(key) {
    return ['profile_image', 'rrn_image', 'representative_qualification_id', 'bank_file'].includes(String(key || '').trim());
}
...
export function renderRepresentativeQualification(row = {}) {
    ...
    const filePath = String(row.representative_qualification_file || '');
    const preview = filePath && /\.(jpe?g|png|gif|webp)$/i.test(filePath)
        ? `/api/file/preview?path=${encodeURIComponent(filePath)}`
        : filePath ? '/public/assets/img/has-cert.png' : '/public/assets/img/placeholder-cert.png';
    $('#edit_representative_qualification_preview').attr('src', preview).attr('data-file-path', filePath);
    $('#edit_representative_qualification_name').val(row.representative_qualification_name || '');
    $('#edit_representative_qualification_delete').val('0');
    $('#edit_representative_qualification_delete_btn').toggle(!!row.representative_qualification_id);
    $('#representative_qualification_box').attr('data-label', filePath ? '원본 보기' : '업로드');
