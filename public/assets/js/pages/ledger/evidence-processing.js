...
(() => {
    ...
    function trashApiUrl(params = {}) {
        const url = new URL(API.trash, window.location.origin);
        const activeType = String(params.import_type ?? selectedTypeFilter ?? '').trim().toUpperCase();
        if (activeType !== '') {
            url.searchParams.set('import_type', activeType);
            url.searchParams.set('data_type', activeType);
        }
        return `${url.pathname}${url.search}`;
    }
    ...
    function normalizeTransactionLine(line = {}, payload = {}) {
        const canonicalAmount = numericValue(line.item_total_amount ?? line.amount ?? line.total_amount ?? line.supply_amount ?? '');
        const canonicalQty = numericValue(line.item_quantity ?? line.item_qty ?? line.quantity ?? '');
        const canonicalPrice = numericValue(line.item_unit_price ?? line.item_price ?? line.unit_price ?? '');
        const canonicalExchangeRate = numericValue(line.exchange_rate ?? payload.exchange_rate ?? '');
        const canonicalForeignPrice = numericValue(line.item_foreign_unit_price ?? line.foreign_unit_price ?? '');
        const canonicalForeignAmount = numericValue(line.item_foreign_amount ?? line.foreign_amount ?? '');
        return {
            line_type: line.transaction_line_type || line.line_type || line.amount_type || 'ITEM',
            item_date: line.item_date || payload.transaction_date || '',
            item_name: line.item_name || '',
            item_spec: line.item_specification || line.item_spec || line.specification || '',
            unit_name: line.item_unit_name || line.unit_name || '',
            item_qty: canonicalQty ?? '',
            currency: currencyDisplayValue(line.currency || ''),
            exchange_rate: canonicalExchangeRate ?? '',
            foreign_unit_price: canonicalForeignPrice ?? '',
            foreign_amount: canonicalForeignAmount ?? '',
            item_price: canonicalPrice ?? '',
            amount: canonicalAmount ?? '',
            item_note: line.item_description || line.item_note || line.description || '',
        };
    }
    ...
    function serializeTransactionLine(line = {}, payload = {}) {
        ...
        const lineTypeCode = codeOptionByValue('TRANSACTION_LINE_TYPE', normalized.line_type)?.code
            || String(normalized.line_type || 'ITEM').trim()
            || 'ITEM';
        const amountValue = numericValue(normalized.amount) || 0;
        const isVatLine = lineTypeCode === 'VAT';
        const isBaseLine = ['ITEM', 'GOODS', 'SERVICE'].includes(lineTypeCode);

        return {
            transaction_line_type: lineTypeCode,
            item_date: normalized.item_date || '',
            item_name: normalized.item_name || '',
            item_specification: normalized.item_spec || '',
            item_unit_name: normalized.unit_name || '',
            item_quantity: normalized.item_qty === '' ? '' : normalized.item_qty,
            item_unit_price: normalized.item_price === '' ? '' : normalized.item_price,
            item_foreign_unit_price: normalized.foreign_unit_price === '' ? '' : normalized.foreign_unit_price,
            item_foreign_amount: normalized.foreign_amount === '' ? '' : normalized.foreign_amount,
            item_supply_amount: isBaseLine ? amountValue : 0,
            item_vat_amount: isVatLine ? amountValue : 0,
            item_total_amount: amountValue,
            item_tax_type: currencyCode && currencyCode !== 'KRW' ? 'ZERO' : 'TAXABLE',
            item_description: normalized.item_note || '',
    ...
    function transactionLineTotalAmount(rows = []) {
        return sanitizeTransactionLineRows(rows)
            .reduce((sum, row) => sum + (numericValue(row?.item_total_amount ?? row?.amount) || 0), 0);
    }
    ...
    function openTrashModal() {
        ...
        const activeType = String(selectedTypeFilter || '').trim().toUpperCase();
        modal.dataset.listUrl = trashApiUrl({ import_type: activeType });
        modal.dataset.restoreUrl = API.restoreRows;
        modal.dataset.deleteUrl = API.purgeRows;
        modal.dataset.deleteAllUrl = API.purgeAll;
        modal.dataset.importType = activeType;
        bootstrap.Modal.getOrCreateInstance(modal, { focus: false }).show();
    }
    ...
    async function updateTrashButtonState() {
        ...
        try {
            const res = await fetch(trashApiUrl(), { credentials: 'same-origin' });
