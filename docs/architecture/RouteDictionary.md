...
|---|---|---|
| `web.main.dashboard` | `/main` | ERP 최상위 Main 도메인의 기본 현황 Dashboard |
| `web.main.report` | `/main/report` | 전사 통합보고서 |
| `web.main.calendar` | `/main/calendar` | 전사 일정·캘린더 화면 |
| `web.main.activity` | `/main/activity` | 전사 최근 활동 화면 |
| `web.main.notifications` | `/main/notifications` | 개인 업무 알림센터 |
| `web.main.kpi` | `/main/kpi` | 전사 실적 현황 |
| `web.main.settings` | `/main/settings` | ERP 공통 설정 진입 화면 |
| `api.main.calendar.*` | `/api/main/calendar/*` | Main 캘린더 조회·저장·삭제·복구 API |
| `api.main.profile_summary` | `/api/main/profile-summary` | Main 사용자 프로필 요약 API |
...
- `GET /api/institution/income-data/daily-employment/template`: 다중 Group·작업자·1~31일 입력양식 다운로드.
- `POST /api/institution/income-data/daily-employment/excel`: 현재 Modal 편집자료 다운로드.
- `POST /api/institution/income-data/daily-employment/excel-upload-preview`: 파일 전체 검증과 Group 생성 Preview. DB 저장 없음.
- `GET /api/institution/income-data/business-income/template`: 사업소득 지급그룹·소득자 평면 입력양식과 활성 참조정보 다운로드.
- `POST /api/institution/income-data/business-income/excel`: 현재 사업소득 Modal 편집자료 다운로드.
- `POST /api/institution/income-data/business-income/excel-upload-preview`: 파일 전체 검증·법정기준 계산 후 지급그룹 Preview 반환. DB 저장 없음.
...
|---|---|---|
| `code.view` | `/main/settings/standard/code` | 기준관리 > 코드관리. 기존 Controller·View·JS와 권한을 그대로 재사용한다. |
...
- 확정 Revision의 직접 변경을 막기 위해 `api.settings.statutory_standards.reorder`와 `api.settings.statutory_standards.delete` 공개 Route는 제거했다. 정정은 `correct-revision`만 사용한다.
...
| `web.institution.human_resources.attendance` | `/institution/human-resources/attendance` | 일별 조회, 월별 누계, 누락·이상 조치, 월 마감·Revision을 탭별로 분리한 관리자 근태 화면. 본인 출퇴근 버튼은 노출하지 않는다. |
| `api.institution.human_resources.attendance.daily_list` | `/api/institution/human-resources/attendance/daily-list` | 직원·근무일별 예정·실제·계산·예외 상태와 상세/재계산 진입 목록. |
| `api.institution.human_resources.attendance.monthly_list` | `/api/institution/human-resources/attendance/monthly-list` | 미마감·재오픈 월은 Daily 실시간 누계, 마감 월은 현재 Closure History snapshot을 제공하는 읽기 전용 월별 누계. |
| `api.institution.human_resources.attendance.exception_list` | `/api/institution/human-resources/attendance/exception-list` | 미해결 예외의 유형·사유·직원·근무일·계산상태와 확인/보정/정정 진입 목록. |
| `api.institution.human_resources.attendance.closure_list` | `/api/institution/human-resources/attendance/closure-list` | 직원·월별 마감 준비상태, 차단 사유, 현재 Revision과 마감/재오픈 진입 목록. 월별 누계와 동일한 Model projection을 재사용한다. |
...
| `api.institution.human_resources.attendance.clock_self` | `/api/institution/human-resources/attendance/clock/self` | 서버가 결정한 본인 직원과 서버 현재시각의 웹 출퇴근 등록. 관리자 근태관리 화면에서는 사용하지 않고 별도 본인 UX 진입점을 위해 유지한다. |
| `api.institution.human_resources.attendance.clock_admin` | `/api/institution/human-resources/attendance/clock/admin` | 직원·출퇴근 유형·과거 발생일시·보정 사유를 입력하는 관리자용 출퇴근 기록 보정. |
...
| `web.institution.human_resources.leave` | `/institution/human-resources/leave` | 관리자용 전체 휴가 현황·부여·잔액·유형 관리. |
| `web.approval.leave-request` | `/approval/leave-request` | 직원 본인 휴가 신청·상신·회수·취소요청 및 결재상태 확인. |

### 휴가 API

- `/api/approval/leave-request/list`, `/options`, `/detail`: 로그인 직원 본인의 신청 목록·선택항목·상세
- `/api/approval/leave-request/save`, `/save-submit`, `/submit`, `/withdraw`, `/cancel-request`: 본인 신청·결재·회수·승인 후 취소요청
- `/api/institution/human-resources/leave/status-list`, `/balance-list`, `/options`, `/detail`: 관리자 현황·잔액·상세
- `/api/institution/human-resources/leave/grant`, `/adjust`, `/type-save`: 관리자 부여·조정·유형 정책
...
- `/api/institution/income-data/regular-employment/save`, `/submit`, `/withdraw`, `/delete`: Header·Item 저장, 공용 결재 상신·회수, 초안계열 소프트삭제
- `/api/institution/income-data/regular-employment/trash`, `/restore`, `/restore-bulk`, `/restore-all`, `/purge`, `/purge-bulk`, `/purge-all`: 공용 휴지통 조회·복원·완전삭제 계약. Evidence·회계·정정 참조 문서는 완전삭제하지 않는다.
| `web.institution.income_data.daily_employment` | `/institution/income-data/daily-employment` | 일용근로소득 목록·작성 화면. |
| `api.institution.income_data.daily_employment.list` | `/api/institution/income-data/daily-employment/list` | 일용근로소득 목록 조회. |
| `api.institution.income_data.daily_employment.detail` | `/api/institution/income-data/daily-employment/detail` | Header·Item·Workday·Line 상세 조회. |
| `api.institution.income_data.daily_employment.options` | `/api/institution/income-data/daily-employment/options` | 조회권한을 확인한 뒤 일용근로소득 기본옵션 또는 근무그룹의 사업구분·프로젝트·작업팀·작업자·공종 AJAX 선택목록을 반환한다. AJAX 목록은 검색어·페이지당 최대 20건·`has_more` 계약을 사용하며 각 원본 페이지의 활성 기준정보를 SSOT로 조회한다. 일용 화면은 거래처관리·코드관리 화면 권한에 의존하지 않으며 선택값의 사업구분·프로젝트·작업자·근무일 배정 적합성은 계산·저장 시 서버에서 별도로 검증한다. |
| `api.institution.income_data.daily_employment.calculate` | `/api/institution/income-data/daily-employment/calculate` | 근무일별 법정기준 계산 Preview. |
| `api.institution.income_data.daily_employment.preflight` | `/api/institution/income-data/daily-employment/preflight` | 저장 문서의 상태·계산 재현성·Header/Item/Workday/Line 합계·법정기준 추적·보험·비과세·중복귀속을 검증하고 상신 가능 여부를 구조화해 반환. 실제 상신이나 상태 변경은 하지 않는다. |
| `api.institution.income_data.daily_employment.save` | `/api/institution/income-data/daily-employment/save` | 수동 입력한 Header·Item·Workday·Line·합계 Projection을 Command 멱등키와 함께 저장. |
| `api.institution.income_data.daily_employment.delete` | `/api/institution/income-data/daily-employment/delete` | 초안·반려·회수 문서 소프트삭제. |
| `api.institution.income_data.daily_employment.trash` | `/api/institution/income-data/daily-employment/trash` | 공용 휴지통 목록 조회. |
| `api.institution.income_data.daily_employment.restore*` | `/api/institution/income-data/daily-employment/restore`, `/restore-bulk`, `/restore-all` | 단건·선택·전체 복원. |
| `api.institution.income_data.daily_employment.purge*` | `/api/institution/income-data/daily-employment/purge`, `/purge-bulk`, `/purge-all` | 단건·선택·전체 완전삭제. Evidence 참조 문서는 제외한다. |
| `web.institution.income_data.business_income` | `/institution/income-data/business-income` | 사업소득 P1 작성·계산·결재 화면. |
...
| `api.settings.system.data_table_columns` | `/api/settings/system/data-table-columns` | Returns canonical DB physical-column metadata only. Labels, order, type, and required flags come from DB COMMENT, `ORDINAL_POSITION`, `DATA_TYPE`, and `IS_NULLABLE`; JOIN, Projection, and virtual UI columns are excluded. |
...
| `api.ledger.voucher.list` | `GET /api/ledger/voucher/list` | 전표입력의 일반 DataTable 목록 조회 경로. Query string 검색조건을 `VoucherController@apiList`에서 처리한다. |
| `api.ledger.voucher.review_list` | `POST /api/ledger/voucher/review-list` | 전표검토·전기 공용 DataTable 서버사이드 목록 조회 경로. DataTables POST 요청에 `scope=review`를 포함하며 `VoucherController@apiList`가 검토 대상 상태의 페이징·검색·정렬 결과를 반환한다. |
| `api.ledger.voucher.evidence_search` | `/api/ledger/voucher/evidence-search` | 전표입력 증빙 추가 공용 DataTable 서버 조회 경로. 활성 `VOUCHER` 대상 `ledger_evidence_links`가 존재하는 증빙과 현재 전표 모달에 추가한 `exclude_evidences`를 제외한다. 저장 전 현재 전표에서 사용자가 연결 해제한 `released_evidences`는 해당 전표의 기존 링크만 검색 예외로 인정하여 즉시 재선택할 수 있으며, 다른 전표의 활성 링크는 계속 제외한다. `TRANSACTION` 링크는 전표 추가목록 제외 조건이 아니다. `evidence_type`, `import_type`, `exclude_evidences`, `released_evidences`, `q`, `page`, `per_page`, column-key 기반 `sort_field`/`sort_direction`을 받고 DataTables draw/전체 건수와 활성 자료유형 필터를 반환한다. |
| `api.ledger.voucher.evidence_recommendations` | `/api/ledger/voucher/evidence-recommendations` | 전표 모달에 현재 추가된 `(import_type, evidence_id)` 전체와 전표 회계일 `accounting_date`를 분석한다. 건별 상태와 선택형 후보, 표준 `line.refs[]` 및 원천 추적 `line.source_refs[]`를 반환한다. 개인경비는 Item별 차변과 동일 집계정책의 직원 대변만 합산하고 건수·금액·identity·보조정보 Coverage를 제공하되 전표·라인·링크를 저장하거나 자동 적용하지 않는다. |
...
| Key | URL | Responsibility |
|---|---|---|
| `api.institution.human_resources.pay_component.options` | `/api/institution/human-resources/pay-component/options` | 근로계약·상용근로소득 공용 적용일 기준 활성 급여항목 옵션과 과세정책 Projection |
| `api.institution.human_resources.pay_component.list` | `/api/institution/human-resources/pay-component/list` | 근로계약 지급조건 급여항목 관리 목록 |
| `api.institution.human_resources.pay_component.detail` | `/api/institution/human-resources/pay-component/detail` | 급여항목 마스터 상세조회 |
| `api.institution.human_resources.pay_component.save` | `/api/institution/human-resources/pay-component/save` | 급여항목 마스터 신규·수정 저장 |
| `api.institution.human_resources.pay_component.delete` | `/api/institution/human-resources/pay-component/delete` | 급여항목 마스터 소프트삭제 |
| `api.institution.human_resources.pay_component.reorder` | `/api/institution/human-resources/pay-component/reorder` | 선택행 상·하 이동 결과를 급여항목 마스터 `sort_no`에 저장 |
...
| `api.institution.human_resources.employment_contract.options` | `/api/institution/human-resources/employment-contract/options` | 신규·상세 Modal의 계약 입력정책·급여항목 조회 및 계약 시작일 기준 최저임금 입력안내 조회 |
| `api.institution.human_resources.employment_contract.statutory_projection` | `/api/institution/human-resources/employment-contract/statutory-projection` | 계약 시작일 기준 법정기준 검증 Projection·Revision·Source 조회 |
...
| `api.institution.human_resources.employment_contract.revise` | `/api/institution/human-resources/employment-contract/revise` | 승인·시행 계약의 조건변경(`CHANGE`) 또는 입력정정(`CORRECTION`) 초안 생성 |
...
| `api.approval.personal-expense.effective-classifications` | `/api/approval/personal-expense/effective-classifications` | 승인 Item 원본분류·Evidence raw 분류·최신 정정 Revision 기반 유효 회계분류를 조회한다. |
| `api.approval.personal-expense.correct-classification` | `/api/approval/personal-expense/correct-classification` | 별도 회계분류 정정 권한으로 최종승인 개인경비 Item의 불변 정정 Revision을 일괄 생성한다. |
...
### 2026-08-21 자격·교육 정책 API

| Key | URL | 책임 |
|---|---|---|
| `api.institution.human_resources.qualification_education.qualification.invalidate` | `/api/institution/human-resources/qualification-education/qualification/invalidate` | 자격 무효화 |
| `api.institution.human_resources.qualification_education.education.invalidate` | `/api/institution/human-resources/qualification-education/education/invalidate` | 교육이력 무효화 |
| `api.institution.human_resources.qualification_education.qualification_type.list/save` | `/api/institution/human-resources/qualification-education/qualification-type/{list|save}` | 자격 종류 정책 조회·저장 |
| `api.institution.human_resources.qualification_education.course.list` | `/api/institution/human-resources/qualification-education/course/list` | 교육과정 정책 조회 |
| `api.institution.human_resources.qualification_education.requirement.list/save` | `/api/institution/human-resources/qualification-education/requirement/{list|save}` | 직무 요구조건 조회·저장 |
| `api.institution.human_resources.qualification_education.policy.reorder` | `/api/institution/human-resources/qualification-education/policy/reorder` | 정책 표시순서 저장 |

기존 직접 Excel 경로는 제거하며 공용 Excel Manager 외의 페이지 전용 다운로드 경로를 만들지 않는다.
### 자격·교육 교육 일정 API

- `POST /api/institution/human-resources/qualification-education/session/list`
- `GET /api/institution/human-resources/qualification-education/session/detail`
- `POST /api/institution/human-resources/qualification-education/session/save`
- `POST /api/institution/human-resources/qualification-education/session/transition`
- `POST /api/institution/human-resources/qualification-education/session-target/list`
- `POST /api/institution/human-resources/qualification-education/session-target/add`
- `POST /api/institution/human-resources/qualification-education/session-target/remove`
- `POST /api/institution/human-resources/qualification-education/session-target/outcome`
- `POST /api/institution/human-resources/qualification-education/session-target/acknowledge`
- 기존 자격·교육 권한 key를 재사용하며 신규 Permission DB row는 만들지 않는다.

### 직원 대표자격 후보 API

- `GET /api/settings/organization/employee/representative-qualifications`
- 해당 직원이 소유한 검증 완료·유효 ACTIVE 자격만 반환한다.

## 2026-08-21 — 공용 알림센터

| Key | URL | 책임 |
|---|---|---|
| `web.main.notifications` | `/main/notifications` | 회사 공지와 분리된 내 Notification Center |
| `api.system.notifications` | `GET /api/system/notifications` | Navbar 최근 알림과 미읽음 수 조회 |
| `api.system.notifications.list` | `GET /api/system/notifications/list` | 본인 Core 알림 서버 Paging |
| `api.system.notifications.read` | `POST /api/system/notifications/read` | 소유권 조건이 포함된 단건 읽음 |
| `api.system.notifications.read_all` | `POST /api/system/notifications/read-all` | 본인 알림 전체 읽음 |

모든 API는 로그인 사용자의 ID를 서버 세션에서 결정한다. Event 생성 Public API와 관리자 수동발송 API는 없다.
# 2026-08-22 상용근로소득 보강 API

- `api.institution.income_data.regular_employment.calculate`: 귀속월·직원목록 기준 일괄 계산 Preview. 세액 기준일은 서버가 귀속월 말일로 확정한다.
- `api.institution.income_data.regular_employment.adjust`: 동적 항목별 조정금액·사유 저장과 Audit.
- `api.institution.income_data.regular_employment.report_dataset`: `REGULAR_EMPLOYMENT_INCOME_PAYROLL_REGISTER` 마스킹 Dataset 조회.
# social-insurance (2026-08-28)

- `web.institution.social_insurance` → `GET /institution/social-insurance`: 메뉴·권한·Breadcrumb를 유지하는 개발예정 Placeholder다. 페이지 전용 직원 목록·Coverage 조회/저장 API는 제공하지 않는다.

## 2026-08-29 일용근로자 고용기간 API

- `api.institution.income_data.daily_employment.employment_period_resolve`: 귀속일 최신 고용기간 leaf 조회.
- `api.institution.income_data.daily_employment.employment_period_save`: 고용기간 신규 확정.
- `api.institution.income_data.daily_employment.employment_period_correct`: 원행 불변 선형 정정.

## 사업소득 P1 Route (2026-09-03)

| Key | URL | 책임 |
|---|---|---|
| `web.institution.income_data.business_income` | `GET /institution/income-data/business-income` | 사업소득 목록·작성·계산·결재 화면 |
| `api.institution.income_data.business_income.list` | `POST /api/institution/income-data/business-income/list` | 사업소득 서버 Paging 목록 |
| `api.institution.income_data.business_income.detail` | `GET /api/institution/income-data/business-income/detail` | Header·Group·Item 상세 |
| `api.institution.income_data.business_income.options` | `GET /api/institution/income-data/business-income/options` | 사업구분·프로젝트·작업팀·소득자 선택지 조회 |
| `api.institution.income_data.business_income.calculate` | `POST /api/institution/income-data/business-income/calculate` | 세무 적격성 및 법정기준 계산 Preview |
| `api.institution.income_data.business_income.preflight` | `GET /api/institution/income-data/business-income/preflight` | 결재요청 전 저장 계산 Revision 무결성 검증 |
| `api.institution.income_data.business_income.save` | `POST /api/institution/income-data/business-income/save` | Aggregate와 계산 Revision 저장 |
| `api.institution.income_data.business_income.submit` | `POST /api/institution/income-data/business-income/submit` | 확정 계산 Revision 결재요청 |
| `api.institution.income_data.business_income.withdraw` | `POST /api/institution/income-data/business-income/withdraw` | 진행 중인 사업소득 기안 회수 |
| `api.institution.income_data.business_income.delete` | `POST /api/institution/income-data/business-income/delete` | 업무문서 Soft Delete |
| `api.institution.income_data.business_income.trash` | `GET /api/institution/income-data/business-income/trash` | 공용 휴지통 목록 |
| `api.institution.income_data.business_income.restore` | `POST /api/institution/income-data/business-income/restore` | 휴지통 문서 복구 |
| `api.institution.income_data.business_income.purge` | `POST /api/institution/income-data/business-income/purge` | 미승인 휴지통 문서 영구삭제 |

- 기존 `BUSINESS_INCOME` Evidence 요청은 API 읽기 경계에서 `BUSINESS_INCOME_REPORT`로만 호환한다. 신규 Evidence와 Link에는 `BUSINESS_INCOME_REPORT`만 기록한다.
### `web.ledger.data.daily-employment-incomes`

- URL: `/ledger/data/daily-employment-incomes`
- Controller: `EvidenceController@webTypePage`
- `DAILY_EMPLOYMENT_INCOME` Canonical Type의 공용 증빙원본 목록 페이지다.
