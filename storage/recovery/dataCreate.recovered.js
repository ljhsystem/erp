...
(() => {
    ...
    function apiFailureMessage(json = {}, fallback = '요청 처리에 실패했습니다.') {
    ...
    async function requestCreateTransactions(ids, extraPayload = {}) {
        ...
        if (!response.ok || json.success === false) {
            throw new Error(apiFailureMessage(json, '거래/전표 생성 요청 처리에 실패했습니다.'));
        }
        return json;
    }
    ...
    function selectionText(value) {
        const text = String(value ?? '').trim();
        return isNoneSelectionValue(text) ? '' : text;
    }
    ...
    function rowProjectName(row) {
        const payload = mapped(row);
        return selectionText(row?.project_name)
            || selectionText(payload.project_name)
            || selectionText(payload.project_code)
            || '';
    }
    ...
    function hasProjectSelection(payload = {}) {
        return selectionText(payload.project_id) !== '' || selectionText(payload.project_name) !== '';
    }
    ...
    function validateBusinessProjectRule(payload = {}, modal = null) {
        ...
        if (type === 'OTHER' && hasProject) {
            ...
            notify('warning', '사업구분이 전문건설업이 아닐 때는 프로젝트를 선택할 수 없습니다.');
            return false;
        }
        if (type === 'CONSTRUCTION' && !hasProject) {
            projectTarget?.classList.add('is-invalid');
            focusReadinessTarget(modal, projectTarget);
            notify('warning', '사업구분이 전문건설업일 때는 프로젝트를 선택해야 합니다.');
            return false;
        }
        return true;
    }
    ...
    function voucherCreateState(row) {
        ...
        if (!hasReadyVoucherLines(row)) return 'JOURNAL_INCOMPLETE';
        return 'READY';
    }
    ...
    function voucherLineValidationIssues(row = {}) {
        const groups = voucherLineGroups(mapped(row));
        return voucherLineGroupValidationIssues(groups);
    }

    function voucherLineGroupValidationIssues(groups = []) {
        if (!hasJournalVoucherLineGroups(groups)) return ['분개라인이 없습니다.'];
        const issues = [];
        let debitTotal = 0;
        let creditTotal = 0;
        groups.forEach((group, index) => {
            const account = String(group.account_id || '').trim();
            const debit = Math.abs(numericValue(group.debit) || 0);
            const credit = Math.abs(numericValue(group.credit) || 0);
            if (!account) issues.push(`${index + 1}번 라인 계정과목 누락`);
            if (!((debit > 0 && credit === 0) || (debit === 0 && credit > 0))) {
                issues.push(`${index + 1}번 라인은 차변 또는 대변 중 하나만 입력해야 합니다.`);
            }
            debitTotal += debit;
            creditTotal += credit;
        });
        if (debitTotal <= 0 || creditTotal <= 0 || Math.round(debitTotal * 100) !== Math.round(creditTotal * 100)) {
            issues.push('차변합계와 대변합계가 일치하지 않습니다.');
        }
        return issues;
    }

    function voucherLineTotalsFromGroups(groups = []) {
        return groups.reduce((totals, group) => {
            totals.debit += Math.abs(numericValue(group.debit) || 0);
            totals.credit += Math.abs(numericValue(group.credit) || 0);
            return totals;
        }, { debit: 0, credit: 0 });
    }

    function voucherLineGroupsFromTable(table) {
        return Array.from(table?.querySelectorAll('tr[data-voucher-line-row]') || []).map((row, index) => ({
            sourceLineNo: normalizeVoucherSourceLineNo(row.dataset.sourceLineNo, index + 1),
            account_id: row.querySelector('.voucher-line-account')?.value?.trim() || '',
            debit: row.querySelector('.voucher-line-debit')?.value || '',
            credit: row.querySelector('.voucher-line-credit')?.value || '',
            line_summary: row.querySelector('.voucher-line-summary')?.value?.trim() || '',
        }));
    }

    function voucherLineSummaryHtml(groups = []) {
        const totals = voucherLineTotalsFromGroups(groups);
        const issues = voucherLineGroupValidationIssues(groups);
        const isOk = issues.length === 0;
        return `
            <div class="voucher-line-summary-panel" data-voucher-line-summary>
                <div class="journal-form-field journal-summary-item">
                    <label class="form-label">차변 합계</label>
                    <input type="text" class="form-control form-control-sm voucher-line-debit-total" value="${escapeHtml(formatNumber(totals.debit) || '0')}" readonly>
                </div>
                <div class="journal-form-field journal-summary-item">
                    <label class="form-label">대변 합계</label>
                    <input type="text" class="form-control form-control-sm voucher-line-credit-total" value="${escapeHtml(formatNumber(totals.credit) || '0')}" readonly>
                </div>
                <div class="journal-form-field journal-summary-item journal-summary-item--status">
                    <label class="form-label">검증 상태</label>
                    <div class="voucher-validation-badge ${isOk ? 'voucher-validation-ok' : 'voucher-validation-error'}" data-voucher-line-balance-status aria-live="polite">
                        ${escapeHtml(isOk ? '차변/대변 합계가 일치합니다.' : issues[0])}
                    </div>
                </div>
            </div>
        `;
    ...
    function syncVoucherLineSummary(table) {
        const container = table?.closest('[data-voucher-lines-table]');
        const summary = container?.querySelector('[data-voucher-line-summary]');
        if (!container || !summary) return;
        const groups = voucherLineGroupsFromTable(table);
        const totals = voucherLineTotalsFromGroups(groups);
        const issues = voucherLineGroupValidationIssues(groups);
        const isOk = issues.length === 0;
        const debitEl = summary.querySelector('.voucher-line-debit-total');
        const creditEl = summary.querySelector('.voucher-line-credit-total');
        const statusEl = summary.querySelector('[data-voucher-line-balance-status]');
        if (debitEl) debitEl.value = formatNumber(totals.debit) || '0';
        if (creditEl) creditEl.value = formatNumber(totals.credit) || '0';
        if (statusEl) {
            statusEl.className = `voucher-validation-badge ${isOk ? 'voucher-validation-ok' : 'voucher-validation-error'}`;
            statusEl.textContent = isOk ? '차변/대변 합계가 일치합니다.' : issues[0];
            statusEl.title = issues.join('\n');
        }
    }

    function hasReadyVoucherLines(row = {}) {
        return voucherLineValidationIssues(row).length === 0;
    }
    ...
    function voucherCreateStatusBadge(row) {
        ...
        const reason = state === 'JOURNAL_INCOMPLETE' ? voucherLineValidationIssues(row).join('\n') : raw;
        const voucherNo = String(row?.bundled_voucher_no || row?.voucher_no || row?.linked_voucher_no || '').trim();
        const bundleCount = Number(row?.bundled_voucher_source_count || row?.linked_voucher_source_count || 0);
        const isBundled = String(row?.is_bundled_voucher || '').trim() === '1'
            || String(row?.voucher_import_type || '').trim().toUpperCase() === 'BUNDLED_EVIDENCE'
            || String(row?.bundled_voucher_id || '').trim() !== ''
            || bundleCount > 1;
        const bundleBadge = isBundled
            ? `<span class="badge seed-status-badge seed-status-bundled ms-1" title="${escapeHtml(`묶음전표 ${voucherNo || ''}${bundleCount > 0 ? ` (${bundleCount}건)` : ''}`)}">묶음 ${escapeHtml(voucherNo || '-')}</span>`
            : '';
        return `<span title="${escapeHtml(reason)}">${voucherStateBadge(state)}${bundleBadge}</span>`;
    }
    ...
    function journalCorrectionIssueItems(row = {}) {
        ...
        return voucherLineValidationIssues(row).map((message) => ({ field: 'line_no', message }));
    }
    ...
    function selectedTransactionReadyIds() {
        ...
        return Array.from(selectedIds).filter((id) => {
            ...
            return transactionCreateState(row) === 'READY';
        });
    }

    function selectedVoucherReadyIds() {
        const rows = evidenceTable?.rows().data().toArray() || [];
        return Array.from(selectedIds).filter((id) => {
            const row = rows.find((item) => String(item.id) === String(id));
            if (!row || normalizedStatus(row) === 'DELETED') return false;
    ...
    function updateButtons() {
        const transactionCount = selectedTransactionReadyIds().length;
        const voucherCount = selectedVoucherReadyIds().length;
        const bundleCount = selectedBundleVoucherIds().length;
        const wrapper = evidenceTable?.table().container();

        wrapper?.querySelector('.btn-create-selected-transactions')?.toggleAttribute('disabled', isCreating || transactionCount === 0);
        wrapper?.querySelector('.btn-create-selected-vouchers')?.toggleAttribute('disabled', isCreating || voucherCount === 0);
    ...
    function selectedRefsFromRow(row) {
        const refs = [];
        row?.querySelectorAll('.line-ref-picker').forEach((select) => {
            const type = String(select.dataset.refType || '').trim().toUpperCase();
            const id = String(select.value || '').trim();
            if (type && id) {
                refs.push({ ref_type: type, ref_id: id });
            }
        });
        try {
    ...
    function serializeVoucherLinesFromModal(modal, next) {
        ...
        const issues = voucherLineGroupValidationIssues(voucherLineGroupsFromTable(table));
        if (issues.length > 0) {
            notify('warning', issues[0]);
            table.querySelector('[data-voucher-line-balance-status]')?.focus?.();
            next._voucher_lines = null;
            return false;
        }
    ...
    async function initReadinessModalControls(modal) {
        ...
        modal?.querySelectorAll('[data-voucher-lines-table]').forEach((table) => syncVoucherLineSummary(table));
    }
    ...
    async function createFromReadinessModal(button, mode) {
        ...
        try {
            ...
            let json = null;
            json = await requestCreateTransactions([row.id], { action: mode === 'voucher' ? 'voucher' : 'transaction' });
            if (json.requires_confirmation && json.confirmation_code === 'EXISTING_VOUCHER') {
                ...
                if (!confirmed) return;
                json = await requestCreateTransactions([row.id], { action: mode === 'voucher' ? 'voucher' : 'transaction', confirm_existing_voucher: true });
            }
    ...
    async function updateTrashButtonState() {
        ...
        try {
            const res = await fetch(`${API.trash}?draw=1&start=0&length=1&sequence_scope=create`, { credentials: 'same-origin' });
            const json = await res.json();
            const rows = json?.success ? (json.data || []) : [];
            const trashCount = Number(json?.recordsFiltered ?? json?.recordsTotal ?? rows.length ?? 0);
            ...
            button.setAttribute('aria-label', hasTrash ? `휴지통 ${trashCount}건` : '휴지통');
            button.title = hasTrash ? `휴지통 ${trashCount}건` : '휴지통';
        } catch (error) {
    ...
    function bindEvents() {
        ...
        document.addEventListener('input', (event) => {
            const input = event.target.closest('#seedRowReadinessModal .voucher-line-amount');
            if (!input) return;
            syncVoucherLineSummary(input.closest('[data-voucher-lines-table]'));
        });

        document.addEventListener('change', (event) => {
            const voucherInput = event.target.closest('#seedRowReadinessModal .voucher-line-account, #seedRowReadinessModal .voucher-line-amount');
            if (voucherInput) {
                syncVoucherLineSummary(voucherInput.closest('[data-voucher-lines-table]'));
            }
        ...
        document.addEventListener('click', (event) => {
            ...
            const newRow = tbody.querySelector('tr[data-voucher-line-row]:last-child');
            renumberVoucherLineRows(table);
            syncVoucherLineSummary(table);
            initVoucherLineSortable(table);
            void initVoucherLineGridControls(newRow || table).catch((error) => notify('error', error.message));
        });
    ...
    async function createSelectedTransactions(button) {
        ...
        try {
            ...
            notify('success', json.message || '선택한 자료의 거래생성을 요청했습니다.');
        } finally {
            isCreating = false;
    ...
    async function createSelectedVouchers(button) {
        ...
        try {
            ...
            notify('success', json.message || '선택한 자료의 전표발행을 요청했습니다.');
        } finally {
            isCreating = false;
            if (button) button.textContent = originalText;
            reloadRows();
    ...
    function initTable() {
        ...
        evidenceTable = createDataTable({
            ...
            buttons: [
                ...
                {
                    text: '거래생성',
                    className: 'btn btn-success btn-sm btn-create-selected-transactions',
                ...
                {
                    text: '전표발행',
                    className: 'btn btn-primary btn-sm btn-create-selected-vouchers',
                    action: (_event, _dt, node) => {
                        void createSelectedVouchers(node?.get(0)).catch((error) => notify('error', error.message));
                    },
                },
                {
                    text: '묶음전표발행',
