...
$projectId=(string)($db->query("SELECT r.ref_id FROM ledger_voucher_line_refs r JOIN ledger_voucher_lines l ON l.id=r.voucher_line_id JOIN ledger_vouchers v ON v.id=l.voucher_id WHERE r.ref_target='PROJECT' AND v.deleted_at IS NULL AND UPPER(v.status) IN ('POSTED','CLOSED') LIMIT 1")->fetchColumn()?:'');$page=$service->getProjectLedgerPage($request,['project_id'=>$projectId]);
