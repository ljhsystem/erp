...
class JournalCandidateRepository
{
    ...
    public function learningPatterns(array $context, int $limit = 30): array
    {
        ...
        $where = [
            ...
            "(failure_type IS NULL OR failure_type = '')",
        ];
        $params = [
            ':transaction_direction' => $context['transaction_direction'],
            ':import_type' => $context['import_type'],
        ...
        if ($context['client_id'] !== '') {
            [$clientSql, $clientParams] = $this->clientIdentityCondition('client_id', $context['client_id'], 'learning');
            $where[] = "({$clientSql} OR client_id IS NULL)";
            $params += $clientParams;
        }
        $stmt = $this->pdo->prepare("
            SELECT voucher_id,
                   SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN final_line_type = 'DEBIT' THEN final_account_id END ORDER BY final_amount DESC), ',', 1) AS debit_account_id,
                   SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN final_line_type = 'CREDIT' THEN final_account_id END ORDER BY final_amount DESC), ',', 1) AS credit_account_id,
                   CASE
                       WHEN transaction_direction = 'OUT' AND SUM(final_line_type = 'DEBIT') > 1
                           THEN SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN final_line_type = 'DEBIT' THEN final_account_id END ORDER BY final_amount ASC), ',', 1)
                       WHEN transaction_direction = 'IN' AND SUM(final_line_type = 'CREDIT') > 1
                           THEN SUBSTRING_INDEX(GROUP_CONCAT(CASE WHEN final_line_type = 'CREDIT' THEN final_account_id END ORDER BY final_amount ASC), ',', 1)
                       ELSE NULL
                   END AS vat_account_id,
                   MAX(journal_rule_id) AS journal_rule_id,
                   COUNT(*) AS event_count,
                   SUM(CASE WHEN is_user_modified = 0 THEN 1 ELSE 0 END) AS accepted_count,
                   SUM(CASE WHEN is_user_modified = 1 THEN 1 ELSE 0 END) AS modified_count,
