...
class TransactionService
{
    ...
    public function createTransaction(array $data): array
    {
        ...
        try {
            ...
            $transactionCode = CodeHelper::next('ledger_transactions');
            ...
            foreach ($items as $index => $item) {
                ...
                if ($itemName === '') {
                    throw new \InvalidArgumentException(($index + 1) . '번 품목명이 비어 있습니다.');
                }

                $itemPayload = [
                    'id' => (string) ($item['id'] ?? UuidHelper::generate()),
                    'code' => CodeHelper::next('ledger_transaction_items'),
                ...
                if (!$this->transactionItemModel->insert($itemPayload)) {
                    throw new \RuntimeException(($index + 1) . '번 거래 품목 저장에 실패했습니다.');
                }
            }
    ...
    public function createVoucherFromTransaction(string $transactionId): array
    {
        ...
        try {
            ...
            if (($transaction['match_status'] ?? '') !== 'matched') {
                return [
                    'success' => false,
                    'message' => '매핑완료 상태의 거래만 전표를 생성할 수 있습니다.',
                ];
            }
            ...
            if ($matchPayload === null) {
                return [
                    'success' => false,
                    'message' => '전표 생성에 필요한 매핑 정보가 없습니다.',
                ];
            }
            ...
            $voucherCode = CodeHelper::next('ledger_vouchers');
            ...
            $voucherPayload = [
                ...
                'status' => 'posted',
                'summary_text' => $matchPayload['summary_text'] ?? $transaction['item_summary'] ?? $transaction['description'] ?? '원본 거래 전표',
            ...
            if (!$this->voucherModel->insert($voucherPayload)) {
                throw new \RuntimeException('전표 헤더 저장에 실패했습니다.');
            }

            foreach ($lines as $index => $line) {
                $linePayload = [
                    'id' => UuidHelper::generate(),
                    'code' => CodeHelper::next('ledger_voucher_lines'),
                ...
                if (!$this->voucherLineModel->insert($linePayload)) {
                    throw new \RuntimeException(($index + 1) . '번 전표 라인 저장에 실패했습니다.');
                }
            }
            ...
            $linkPayload = [
                'id' => UuidHelper::generate(),
                'code' => CodeHelper::next('ledger_transaction_links'),
                'transaction_id' => $transactionId,
                'voucher_id' => $voucherId,
                'link_type' => 'MANUAL',
                'is_active' => 1,
            ...
            if (!$this->transactionLinkModel->insert($linkPayload)) {
                throw new \RuntimeException('거래-전표 연결 저장에 실패했습니다.');
            }
            ...
            if (!$this->transactionModel->update($transactionId, [
                ...
                throw new \RuntimeException('거래 상태 변경에 실패했습니다.');
            }
    ...
    private function validateTransactionAmounts(array $data): void
    {
        ...
        if ($supplyAmount <= 0 && $vatAmount <= 0 && $totalAmount <= 0) {
            throw new \InvalidArgumentException('거래 금액이 없습니다.');
        }
    }
    ...
    private function normalizeMatchedLines(array $lines): array
    {
        if (!is_array($lines) || $lines === []) {
            throw new \InvalidArgumentException('전표 라인 정보가 없습니다.');
        }
        ...
        foreach ($lines as $index => $line) {
            ...
            if ($accountCode === '') {
                throw new \InvalidArgumentException(($index + 1) . '번 라인의 account_code가 없습니다.');
            }

            if ($debit <= 0 && $credit <= 0) {
                throw new \InvalidArgumentException(($index + 1) . '번 라인의 금액이 없습니다.');
            }

            if ($debit > 0 && $credit > 0) {
                throw new \InvalidArgumentException(($index + 1) . '번 라인은 차변과 대변을 동시에 입력할 수 없습니다.');
            }
        ...
        if (round($debitSum, 2) !== round($creditSum, 2)) {
            throw new \InvalidArgumentException('차변 합계와 대변 합계가 일치하지 않습니다.');
        }
    ...
    private function normalizePayments(array $payments): array
    {
        ...
        foreach ($payments as $index => $payment) {
            ...
            if ($paymentType === '' || $paymentId === '' || $amount <= 0) {
                throw new \InvalidArgumentException(($index + 1) . '번 결제 정보가 올바르지 않습니다.');
            }
