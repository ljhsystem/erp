...
class TransactionService
{
    ...
    public function createTransaction(array $data): array
    {
        ...
        try {
            ...
            if (!$this->transactionModel->insert($transactionPayload)) {
                throw new \RuntimeException('?轅몄뫅???????????얜∥???????怨뚯댅???????????낆젵.');
            }
            ...
            foreach ($items as $index => $item) {
                ...
                if (!$this->transactionItemModel->insert($itemPayload)) {
                    throw new \RuntimeException(($index + 1) . '???轅몄뫅?????????癲???????얜∥???????怨뚯댅???????????낆젵.');
                }
            }
    ...
    public function updateDocumentStatus(string $transactionId, string $docStatus): array
    {
        ...
        try {
            ...
            if (!$updated) {
                return [
                    'success' => false,
                    'message' => '癲ル슣鍮섌뜮?????ㅺ컼???怨뚮뼚??濡ろ뜑??듦덩?????됰꽡???怨?????덊렡.',
                ];
            }
    ...
    public function createVoucherFromTransaction(string $transactionId): array
    {
        ...
        try {
            if ($this->resolveOperationStage() !== self::STAGE_OPERATIONAL) {
                return [
                    'success' => false,
                    'message' => '???⑤㈇猿 ??影?됀????節뉗땡?癲꾧퀗??????れ삀?? ??ш낄援????獄쏅똻?????????????怨?????덊렡.',
                ];
            }
            ...
            if ($existingLinks !== []) {
                return [
                    'success' => false,
                    'message' => '???? ???ㅼ뒦?????ш낄援?遺밸쨬??쎛 ???怨쀪퐨 ??⑤베堉? ??獄쏅똻????????⑤８?????덊렡.',
                ];
            }
            ...
            foreach ($payments as $payment) {
                ...
                if (!$this->voucherPaymentModel->insert($paymentPayload)) {
                    throw new \RuntimeException('??ш낄援???濡ろ뜏????嚥▲꺂???????묎덩?????됰꽡???怨?????덊렡.');
                }
            }
            ...
            if ($this->transactionLinkModel->existsLink($transactionId, $voucherId)) {
                throw new \RuntimeException('??? ?怨뚭퍙??椰꾧퀡???袁るご??낅빍??');
            }
            ...
            try {
                ...
                if (($e->getCode() ?? '') === '23000') {
                    throw new \RuntimeException('??? ?怨뚭퍙??椰꾧퀡???袁るご??낅빍??', 0, $e);
                }
                throw $e;
            }
            ...
            if (!$this->transactionModel->update($transactionId, [
                ...
                throw new \RuntimeException('癲꾧퀗???????ㅺ컼???怨뚮뼚??濡ろ뜑??듦덩?????됰꽡???怨?????덊렡.');
            }
    ...
    private function normalizeMatchedLines(array $lines): array
    {
        ...
        foreach ($lines as $index => $line) {
            ...
            if ($accountCode === '') {
                throw new \InvalidArgumentException(($index + 1) . '????嚥싲갭큔????account_code???ル봿?? ?????욱룏???????낆젵.');
            }
            ...
            if ($debit > 0 && $credit > 0) {
                throw new \InvalidArgumentException(($index + 1) . '????嚥싲갭큔???? ?轅붽틓?蹂잛젂?④낮釉?????????ㅼ뒧??????????덈Ъ???????⑤챷竊?????????욱룏???????낆젵.');
            }
        ...
        if (round($debitSum, 2) !== round($creditSum, 2)) {
            throw new \InvalidArgumentException('?轅붽틓?蹂잛젂?④낮釉?? ????룸챸猷??? ?????ㅼ뒧?? ????룸챸猷??棺??源?? ??嚥싲갭큔?딆뼍留??? ???????????낆젵.');
        }
