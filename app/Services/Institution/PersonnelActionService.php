...
class PersonnelActionService
{
    ...
    public function apply(string $id): array { $result=$this->applyService->apply($id);return ['success'=>true,'data'=>$result,'message'=>$result['already_applied']?'이미 적용된 발령입니다.':'인사발령을 적용했습니다.']; }
    public function delete(string $id): array { [, $actor]=$this->identity();if(!$this->repository->softDelete($id,$actor))throw new \RuntimeException('결재 이력이 없는 작성중 발령만 삭제할 수 있습니다.');return ['success'=>true,'message'=>'휴지통으로 이동했습니다.']; }
    public function restore(string $id): array { [, $actor]=$this->identity();if(!$this->repository->restore($id,$actor))throw new \RuntimeException('복원할 수 없는 발령입니다.');return ['success'=>true,'message'=>'복원했습니다.']; }
    ...
    private function transaction(callable $callback): array { $owned=!$this->pdo->inTransaction();$savepoint='personnel_action_save';if($owned)$this->pdo->beginTransaction();else$this->pdo->exec("SAVEPOINT {$savepoint}");try{$r=$callback();if($owned)$this->pdo->commit();else$this->pdo->exec("RELEASE SAVEPOINT {$savepoint}");return$r;}catch(\Throwable$e){if($owned&&$this->pdo->inTransaction())$this->pdo->rollBack();elseif(!$owned&&$this->pdo->inTransaction()){$this->pdo->exec("ROLLBACK TO SAVEPOINT {$savepoint}");$this->pdo->exec("RELEASE SAVEPOINT {$savepoint}");}if(!$e instanceof \InvalidArgumentException)$this->logger->error('인사발령 트랜잭션 실패',['exception'=>get_class($e),'message'=>$e->getMessage(),'code'=>$e->getCode(),'trace'=>$e->getTraceAsString()]);throw$e;} }
}
