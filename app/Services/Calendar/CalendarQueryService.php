<?php
// 경로: PROJECT_ROOT . '/app/services/calendar/CalendarQueryService.php'
...
private function mapEventRow(array $row): array
{
    ...
    if ($isAllDay) {
        ...
        $end = null;
        if ($endInclusive) {
            $end = CalendarTime::parseLocal($endInclusive)
                ->modify('+1 day')
                ->format('Y-m-d');
        } else {
            // end가 없으면 start 기준 1일짜리로 내려줌 (exclusive)
            $end = $start
                ? CalendarTime::parseLocal($start)->modify('+1 day')->format('Y-m-d')
                : null;
        }
    ...
    if (!empty($recurrence['rrule'])) {
        ...
        if ($freq) {
            ...
            if (!$isAllDay && $start && $end) {
                ...
                $startDt = CalendarTime::parseLocal($row['dtstart']);
                $endDt   = CalendarTime::parseLocal($row['dtend']);
                ...
                $startDt = CalendarTime::parseLocal($start);
                $endDt   = CalendarTime::parseLocal($end);
    ...
    private function mapTaskRow(array $row): array
    {
        ...
        if ($due) {
            ...
            if ($isDateOnly) {
                ...
                $dt = CalendarTime::parseLocal($due);
