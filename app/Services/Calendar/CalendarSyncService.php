<?php
// 경로: PROJECT_ROOT . '/app/services/calendar/CalendarSyncService.php'
namespace App\Services\Calendar;

use PDO;
use App\Models\Dashboard\DashboardCalendarListModel;
use App\Models\Dashboard\DashboardCalendarEventModel;
use App\Models\Dashboard\DashboardCalendarTaskModel;
use App\Models\Dashboard\DashboardCalendarVisibilityModel;
...
class CalendarSyncService
{
    ...
    private DashboardCalendarListModel $listModel;
    private DashboardCalendarEventModel $eventModel;
    private DashboardCalendarTaskModel $taskModel;
    private DashboardCalendarVisibilityModel $visibilityModel;
    ...
    public function __construct(PDO $pdo)
    {
        ...
        $this->listModel  = new DashboardCalendarListModel($pdo);
        $this->eventModel = new DashboardCalendarEventModel($pdo);
        $this->taskModel = new DashboardCalendarTaskModel($pdo);
        $this->externalAccount = new UserExternalAccountModel($pdo);
        $this->visibilityModel = new DashboardCalendarVisibilityModel($pdo);
        $this->logger     = LoggerFactory::getLogger('service-calendar.CalendarSyncService');

        // ✅ 생성자 진입 로그 (이게 안 찍히면 로거/권한 문제)
        $this->logger->info('[CTOR] CalendarSyncService constructed', [
            'sapi' => PHP_SAPI,
    ...
    * ========================================================= */
    ...
    ): array
    {
        ...
        if ($allDay && $dtstartRaw && $dtendRaw) {
            ...
            $dtend = CalendarTime::parseLocal($endDate)
                ->modify('-1 day')
                ->format('Ymd');
    ...
    ): array
    {
        ...
        * =============================== */
        if ($dueRaw) {
            ...
            if ($dueIsDateParam || preg_match('/^\d{8}$/', (string)$dueRaw)) {
                ...
                $dt = CalendarTime::parseLocal((string)$dueRaw);
        ...
        * =============================== */
        if ($startRaw) {
            ...
            $dt = CalendarTime::parseLocal((string)$startRaw);
