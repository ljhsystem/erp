...
class CalendarCrudService
{
    ...
    private SystemSettingConfigModel $systemConfig;
    private UserExternalAccountModel $externalAccount;
    private ExternalAccountService $accountService;
    
    private CalendarIcsService $ics;
    private ?CalendarSyncService $sync = null;
    private ?CalDavClient $caldavClient = null;
    private $logger;

    private function sync(): CalendarSyncService
    {
        if ($this->sync === null) {
            $this->sync = new CalendarSyncService($this->pdo);
        }
        return $this->sync;
    }
    ...
    {
        $this->pdo             = $pdo;
        $this->systemConfig    = new SystemSettingConfigModel($pdo);
        $this->externalAccount = new UserExternalAccountModel($pdo);
        $this->accountService  = new ExternalAccountService($pdo);
        $this->ics             = new CalendarIcsService();    
     ...
     * - CalendarSyncService / Write-back 전용
    ...
    * ========================================================= */
        ...
        return $this->runAndTrack(function () use ($payload) {
            ...
            * ------------------------------------------------- */
            ...
            if ($isAllDay) {
                ...
                $dtendYmd = CalendarTime::parseLocal($baseEnd)
                ...
                $startLocal = CalendarTime::parseLocal($startRaw);

                $endLocal = $endRaw
                    ? CalendarTime::parseLocal($endRaw)
            ...
            * ------------------------------------------------- */
            ...
            if ($isAllDay) {
                ...
                $dtend = CalendarTime::parseLocal($baseEnd)
                        ->modify('+1 day')
                        ->format('Ymd');
            } else {
    ...
    private function rebuildEvent(array $row, array $payload): array
    {
        ...
        * ------------------------------------------------- */
        ...
        if ($isAllDay) {
            ...
            $dtendYmd = CalendarTime::parseLocal($endDate)
                ->modify('+1 day')
                ->format('Ymd');
            ...
            $tzid = (string)($this->systemConfig->get('timezone') ?: CalendarTime::TZID);
            ...
            // 🔥 반드시 서울시간으로 정규화
            $startLocal = CalendarTime::parseLocal($startRaw);
            $endLocal   = CalendarTime::parseLocal($endRaw);
        ...
        * ========================================================= */
        ...
        if (!empty($payload['rrule'])) {
            ...
            if ($payloadIsAllDay) {
                ...
                $tzid = $tzid ?: (string)($this->systemConfig->get('timezone') ?: CalendarTime::TZID);
    ...
    public function createTask(array $payload): array
    {
        return $this->runAndTrack(function () use ($payload) {
            ...
            $tasks = (new \App\Services\Calendar\CalendarQueryService($this->pdo))
        ...
        return $this->runAndTrack(function () use ($payload) {
            ...
            * ------------------------------------------------- */
            ...
            if (!$row) {
                ...
                $sync = new CalendarSyncService($this->pdo);
        ...
        return $this->runAndTrack(function () use ($uid, $calendarId, $completed) {
            ...
            * ------------------------------------------------- */
            ...
            if (!$originIcs) {
                ...
                $sync = new \App\Services\Calendar\CalendarSyncService($this->pdo);
    ...
    * ========================================================= */
    ...
    private function processDueTime(string $dueRaw, bool $isAllDay): array
    {
        ...
        $dt = CalendarTime::parseLocal($dueRaw);

        if ($isAllDay) {

            $rawLines[] = 'DUE;VALUE=DATE:' .
                CalendarTime::toIcsDate($dueRaw);
        
            $payloadForDb['due'] =
                CalendarTime::parseLocal($dueRaw)->format('Y-m-d');
        
        } else {
        
            $rawLines[] = 'DUE;TZID=' . CalendarTime::TZID . ':' .
                CalendarTime::toIcsLocal($dueRaw);
        
            $payloadForDb['due'] =
                CalendarTime::toDbLocal($dueRaw);
        }
