...
LEFT JOIN ledger_data_format_columns existing
 ...
 AND existing.system_field_name = 'source_card_company_name'
SET c.system_field_name = 'source_card_company_name'
WHERE f.data_type = 'CARD_HOMETAX'
  AND REPLACE(TRIM(COALESCE(c.excel_column_name, '')), ' ', '') IN ('카드사', '카드사명', '카드회사')
  AND c.system_field_name IN ('client_id', 'client_name', 'card_name', 'card_company_name')
