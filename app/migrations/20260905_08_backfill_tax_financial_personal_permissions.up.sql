...
SELECT UUID(), profile.user_id, api_permission.id, CURRENT_TIMESTAMP,
       'SYSTEM:MIGRATION:TAX_FINANCIAL_PERMISSION_BACKFILL'
...
JOIN `auth_permissions` web_permission
 ...
 AND web_permission.permission_key LIKE 'web.ledger.tax.%'
JOIN `auth_permissions` api_permission
  ON api_permission.permission_key LIKE 'api.ledger.tax_financial.%'
