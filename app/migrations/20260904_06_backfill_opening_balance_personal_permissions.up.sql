...
SELECT UUID(), profile.user_id, api_permission.id, CURRENT_TIMESTAMP,
       'SYSTEM:MIGRATION:OPENING_BALANCE_PERMISSION_BACKFILL'
...
JOIN `auth_permissions` web_permission
 ...
 AND web_permission.permission_key='web.ledger.settings.opening_balances'
JOIN `auth_permissions` api_permission
  ON api_permission.permission_key LIKE 'api.ledger.opening_balance.%'
