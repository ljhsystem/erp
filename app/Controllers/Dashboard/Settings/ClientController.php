...
class ClientController
{
    ...
    public function apiSave(): void
    {
        ...
        try {
            ...
            $deleteBank = $_POST['delete_bank_file'] ?? '0';

            if ($deleteBank === '1') {

                if (!empty($before) && !empty($before['bank_file'])) {
                    $this->file->delete($before['bank_file']);
                }

                $payload['bank_file'] = null;
            }
            ...
            if (
                isset($_FILES['bank_file']) &&
                $_FILES['bank_file']['error'] === UPLOAD_ERR_OK
            ) {
                ...
                $payload['bank_file'] = $upload['db_path'];
            }
            ...
            if (!empty($before)) {
                ...
                if (!array_key_exists('bank_file', $payload)) {
                    $payload['bank_file'] =
