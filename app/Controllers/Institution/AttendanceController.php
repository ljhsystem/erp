...
class AttendanceController
{
    ...
    public function webIndex(): void{$capabilities=$this->capabilities();ob_start();require PROJECT_ROOT.'/app/views/institution/attendance/index.php';$content=ob_get_clean();(new LayoutController($this->db))->render(['pageTitle'=>'근태관리','content'=>$content,'pageStyles'=>$pageStyles??'','pageScripts'=>$pageScripts??'']);}
    public function apiDailyList(): void{$q=\Core\Helpers\DataTableRequestHelper::input();$scope=$this->scope();$this->releaseSession();$this->respond(fn()=>$this->service->dailyList($q,$scope));}
    public function apiMonthlyList(): void{$q=\Core\Helpers\DataTableRequestHelper::input();$scope=$this->scope();$this->releaseSession();$this->respond(fn()=>$this->service->monthlyList($q,$scope));}
    public function apiClosureList(): void{$q=\Core\Helpers\DataTableRequestHelper::input();$scope=$this->scope();$this->releaseSession();$this->respond(fn()=>$this->service->closureList($q,$scope));}
    public function apiExceptionList(): void{$q=\Core\Helpers\DataTableRequestHelper::input();$scope=$this->scope();$this->releaseSession();$this->respond(fn()=>$this->service->exceptionList($q,$scope));}
    ...
    public function apiOptions(): void{$this->releaseSession();$this->respond(fn()=>$this->service->options());}
