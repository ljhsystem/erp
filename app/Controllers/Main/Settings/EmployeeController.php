...
class EmployeeController
{
    ...
    public function apiSearchPicker(): void
    {
        ...
        try {
            ...
            $rows = $this->service->searchPicker($keyword, 20, $includeInactive);
    ...
    public function apiRepresentativeQualifications(): void
    {
        header('Content-Type: application/json; charset=UTF-8');
        Session::write();
        try {
            ...
            if ($employeeId === '') {
                throw new \InvalidArgumentException('직원 ID가 필요합니다.');
            }
            echo json_encode(['success' => true, 'data' => $this->service->representativeQualifications($employeeId)], JSON_UNESCAPED_UNICODE);
        } catch (\Throwable $e) {
            echo json_encode(['success' => false, 'data' => [], 'message' => '대표자격 후보 조회 중 오류가 발생했습니다.'], JSON_UNESCAPED_UNICODE);
        }
        exit;
    }
    ...
    public function apiSave(): void
    {
        ...
        try {
            $payload = [
                ...
                'representative_qualification_id' => trim($_POST['representative_qualification_id'] ?? ''),
