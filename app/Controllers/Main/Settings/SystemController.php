...
class SystemController
{
    ...
    public function __construct()
    {
        ...
        $this->systemLogService = new SystemLogService(LOGS_DIR);
    }
    ...
    {
        include PROJECT_ROOT . '/app/views/main/settings/system/site.php';
    }
    ...
    {
        include PROJECT_ROOT . '/app/views/main/settings/system/session.php';
    }
    ...
    {
        include PROJECT_ROOT . '/app/views/main/settings/system/security.php';
    }
    ...
    {
        include PROJECT_ROOT . '/app/views/main/settings/system/api.php';
    }
    ...
    {
        include PROJECT_ROOT . '/app/views/main/settings/system/external_services.php';
    }
    ...
    {
        include PROJECT_ROOT . '/app/views/main/settings/system/storage.php';
    }

    public function webDatabase()
    {
        include PROJECT_ROOT . '/app/views/main/settings/system/databasebackup.php';
    }
    ...
    {
        include PROJECT_ROOT . '/app/views/main/settings/system/logs.php';
    }
    ...
    public function apiLogView()
    {
        try {
            ...
            $this->respondJson(['success'=>true,'data'=>$this->systemLogService->view((string)($input['file']??''))]);
        } catch (\InvalidArgumentException|\RuntimeException) {
            $this->respondJson(['success'=>false,'message'=>'로그 파일을 확인할 수 없습니다.'],400);
        } catch (\Throwable) {
            $this->respondJson(['success'=>false,'message'=>'로그 조회 중 오류가 발생했습니다.'],500);
        }
    }
    ...
    public function apiLogDelete()
    {
        try {
            $input = json_decode((string) file_get_contents('php://input'), true) ?: [];
            $this->systemLogService->delete((string)($input['file']??''),ActorHelper::user());
            $this->respondJson(['success'=>true,'message'=>'로그 파일을 삭제했습니다.']);
        } catch (\InvalidArgumentException|\RuntimeException) {
            $this->respondJson(['success'=>false,'message'=>'로그 파일을 삭제할 수 없습니다.'],400);
        } catch (\Throwable) {
            $this->respondJson(['success'=>false,'message'=>'로그 삭제 중 오류가 발생했습니다.'],500);
        }
    }
    ...
    {
        try {
            $count=$this->systemLogService->deleteAll(ActorHelper::user());
            $this->respondJson(['success'=>true,'deleted'=>$count,'message'=>'전체 로그를 삭제했습니다.']);
        } catch (\Throwable) {
            $this->respondJson(['success'=>false,'message'=>'전체 로그 삭제 중 오류가 발생했습니다.'],500);
        }
    }

    public function webLogDownload()
    {
        try{$path=$this->systemLogService->downloadPath((string)($_GET['file']??''));$file=basename($path);header('Content-Type: text/plain; charset=utf-8');header('Content-Disposition: attachment; filename="'.$file.'"');header('Content-Length: '.filesize($path));readfile($path);exit;}
        catch(\InvalidArgumentException|\RuntimeException){http_response_code(404);exit('로그 파일을 찾을 수 없습니다.');}
        catch(\Throwable){http_response_code(500);exit('로그 다운로드 중 오류가 발생했습니다.');}
    }
}
